...
September 11 installed-release acceptance: a WSL child received focus-out `ESC[O` and focus-in `ESC[I` across real focus transitions after a burst. Native title and external UIA reached the final marker while unfocused, before activation or capture. Child-disabled focus reporting stayed silent. Ordinary window close and natural child exit both completed with code zero while NVDA was running. The user authorized local replacement; the previous executable was backed up.

Public release preparation remains separate from local installation. Physical desktop/Magnifier agreement and actual NVDA speech under the user's normal workload remain open under T-021. These tests do not establish uninterrupted physical display freshness or resolve the exact historic activation sequence.
